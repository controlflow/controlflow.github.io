...
На такие случаи спецификация C# вводит понятие выражений, *классифицированных-как-переменная* (*classified as a variable*), чем-то похожее на понятие "lvalue". Однако с этим понятием сопряжено множество нюансов. Например, временные значения (типа возвращаемого значения вызова метода) и `readonly`-поля (вне конструктора) *реклассифицируется* как переменные, чтобы на них все равно можно было вызывать члены структур (вызовы происходят на копии оригинального значения). Со всеми этими нюансами можно разобраться на практике раскурив этот код под отладчиком:
...
Но C# ничего не знает о поведении методов (изменяют-ли они значение), поэтому вынужден всегда разрешать их вызовы.




На самом деле, C# мог бы прикинуться языком более высокого уровня и 




Всех описанных выше проблем можно избежать, если делать структуры C# неизменяемыми - тогда становится глубоко все равно, на копии или адресе что-то вызвали, все работает одинаково.
